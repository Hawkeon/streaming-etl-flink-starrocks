...
def parse_args():
    ...
    parser.add_argument("--data-path", type=str, default=None, help="Path to dataset CSV (default: /app/data/dataset.csv)")
...
class ThroughputProducer:
    def __init__(
        ...
        self._load_track_ids(data_path)

    def _load_track_ids(self, data_path: Optional[str] = None) -> None:
        p = data_path or "/app/data/dataset.csv"
        try:
            df = pl.read_csv(p)
    ...
    async def connect(self) -> None:
        ...
        for i in range(10):
            try:
                ...
                self._table = await self._conn.get_table(fluss.TablePath("fluss", "fluss_user_logs"))
